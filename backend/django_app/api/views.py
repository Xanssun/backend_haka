...
class GameViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if len(all_memory_cards) < cards_quantity:
        ...
        selected_memory_cards = sample(list(all_memory_cards), cards_quantity // 2)
        pairs = list(combinations(selected_memory_cards, 2))

        for card in selected_memory_cards:
            pairs = MemoryCard.objects.filter(name=card.name).exclude(id=card.id).first()
            game.memorycards.add(card, pairs)
